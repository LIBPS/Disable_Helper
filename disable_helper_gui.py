...
def main_loop_nogui(display, actions, keys, settings):
	clear_screen()
	filedialog
	...
	main_loop_nogui(display, actions, keys, settings)

def setup():
	messagebox.showwarning("禁用助手GUI", f"此版本是一个tkinter 0基础的人\n一边学一边做的\n目前建议使用控制台版本")

	try:
		config = parse_config()
	except Exception as Error:
		messagebox.showerror("禁用助手GUI", f"[错误] 解析配置文件时出现错误: {Error}\n删除配置文件可能会解决此问题")
		exit()
	...
	main_loop_nogui(display, actions, keys[settings["language"]], settings)
