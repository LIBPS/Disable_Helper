...
class Action(Config):
	def __init__(self, action) -> None:
		...
		self.type = action["type"].lower()
		self.prefix = action["prefix"]
		self.assets = action["assets"]
	
	def do(self):
		if self.type in self.ACTION_FUNCTION.keys():
			return self.ACTION_FUNCTION[self.type]()
